...
- `POST /api/internal/queue/dispatch` (own secret, `QUEUE_DISPATCH_SECRET`, meant for an external scheduler — **no cron is configured yet**, see below) picks up due jobs, locks each with a conditional `UPDATE ... WHERE status='queued' AND locked_at IS NULL`, re-checks the lead's opt-out/eligibility status immediately before "sending", and enforces the campaign's `daily_limit`. A per-job exception (network blip, provider timeout) no longer aborts the whole batch or leaves the job stuck locked forever — it's caught, the job is unlocked back to `queued` for the next run, and the batch continues.
- **Known limitation — `daily_limit` is not atomic across overlapping dispatcher runs.** The per-job lock (`locked_at IS NULL`) prevents the same job from being sent twice, but the daily budget itself (`loadDailyBudget` in `queue.ts`) is a snapshot read once per `dispatchDueJobs()` call, held in memory for that call only. If the external scheduler ever invokes the dispatcher concurrently (e.g. a cron trigger firing again before a slow previous run finished), two overlapping runs can each see room under `daily_limit` and jointly send past it. **Configure the external scheduler for non-overlapping invocations** (most cron providers, including Vercel Cron, do not run a new invocation while a previous one for the same job is still in flight, but this must be confirmed for whatever scheduler is actually used) — this is a required part of the go/no-go checklist below, not an optional hardening step.
...
- [ ] Scheduler confirmed to invoke `POST /api/internal/queue/dispatch` **non-overlapping** (see "Known limitation" note above — `daily_limit` is not enforced atomically across concurrent runs)
