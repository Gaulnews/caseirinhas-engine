...
- **No real message is ever sent.** `src/lib/server/messaging/provider.ts` ships only a `NullMessagingProvider` that returns `messaging_provider_not_configured` for every job, which the dispatcher records as `skipped`. Wiring a real WhatsApp provider back in is a distinct, separate decision this change does not make.
